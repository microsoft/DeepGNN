...
class GCNDataset(IterableDataset):
    """Dataset to fetch graph data for the model. Implements IterableDataset such that
    inputs and outputs are numpy arays. Torch converts them to tensors in DataLoader wrapping this dataset.
    ...
    def __iter__(self):
        """Create iterator over inputs. We shuffle the inputs at the beginning of each epoch and drop last nodes
        not fitting into a batch."""
