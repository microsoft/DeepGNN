...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--main_path", type=str, required=True, help="Path to main file that contains run_dist."
    )
    parser.add_argument(
        "--hvd", action="store_true", default=False, help="Convert to Horovod trainer instead of standard DDP trainer."
    )
    ...
    if dirname == "":
        dirname = "."
    with open(os.path.join(os.path.dirname(__file__), "data", "ray_util.py"), "r") as file:
        ray_util = file.read()
    if args.hvd:
