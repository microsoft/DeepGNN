...
template <typename UniformSampleNeighborsReply>
void MergeUniformSampledNeighbors(const UniformSampleNeighborsReply &reply, std::mutex &mtx, size_t count,
                                  std::span<NodeId> output_neighbors, std::span<Type> output_types,
                                  std::span<snark::Timestamp> output_edge_created_ts, std::span<const NodeId> node_ids,
                                  snark::Xoroshiro128PlusGenerator &engine, std::span<size_t> shard_counts,
                                  NodeId default_node_id, Type default_type, bool return_edge_created_ts)
{
    if (reply.node_ids().empty())
    {
        return;
