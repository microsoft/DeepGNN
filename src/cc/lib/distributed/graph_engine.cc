    ...
    : m_metadata(std::move(metadata))
{
    ...
    for (size_t partition_index = 0; partition_index < paths.size(); ++partition_index)
    {
        ...
        std::uint64_t start_index = m_partitions.size();
        m_partitions.resize(m_partitions.size() + suffixes.size());
        for (size_t i = 0; i < suffixes.size(); ++i)
        {
            ReadNodeMap(paths[partition_index], suffixes[i], partition_index);
        }

        ThreadPool::GetInstance().RunInParallel(suffixes, [this, &start_index, &storage_type, &paths,
                                                           &partition_index](std::size_t index, std::string content) {
            m_partitions[start_index + index] =
                std::make_shared<Partition>(m_metadata, paths[partition_index], content, storage_type);
        });
    }
}
                                                     ...
                                                     snark::NodeFeaturesReply *response)
{
    ...
    auto output = response->mutable_feature_values();
    output->resize(request->node_ids().size() * fv_size);

    std::mutex mutex;
    // callback function to calculate a portion of the nodes in the full node_ids and then
    // process these nodes.
    auto func = [this, &mutex, request, response, fv_size, &features, &output](const std::size_t &start_node_id,
        ...
        for (std::size_t node_offset = start_node_id; node_offset < end_node_id; ++node_offset)
        {
            ...
            for (size_t partition = 0; partition < partition_count; ++partition, ++index)
            {
                ...
                {
                    auto data = reinterpret_cast<uint8_t *>(output->data());
                    auto data_span = std::span(data + (node_offset * fv_size), fv_size);
                                                     ...
                                                     snark::EdgeFeaturesReply *response)
{
    ...
    auto output = response->mutable_feature_values();
    output->resize(len * fv_size);

    std::mutex mutex;
    // callback function to calculate a part of the input_edge_src list.
    auto func = [this, &mutex, request, fv_size, &features, len, &output, response](const std::size_t &start_node_id,
                                                                                    const std::size_t &end_node_id) {
        for (size_t node_offset = start_node_id; node_offset < end_node_id; ++node_offset)
        {
            ...
            for (size_t partition = 0; partition < partition_count && !found_edge; ++partition, ++index)
            {
                found_edge = m_partitions[m_partitions_indices[index]]->GetEdgeFeature(
                    ...
                    features, std::span(data + node_offset * fv_size, fv_size));
            }
            if (found_edge)
            {
                std::lock_guard<std::mutex> lock{mutex};
                                                             ...
                                                             snark::WeightedSampleNeighborsReply *response)
{
    ...
    size_t node_size = request->node_ids().size();

    response->mutable_shard_weights()->Resize(node_size, {});
    ...
    auto neighbor_func = [this, &request, &count, &response, &mutex, &seed,
        ...
        for (size_t node_index = start_node_id; node_index < end_node_id; ++node_index)
        {
            const auto node_id = request->node_ids()[node_index];
            auto internal_id = m_node_map.find(node_id);
            size_t offset = node_index * count;

            std::fill_n(response->mutable_neighbor_ids()->mutable_data() + offset, count, request->default_node_id());
            std::fill_n(response->mutable_neighbor_types()->mutable_data() + offset, count,
                        request->default_edge_type());
            std::fill_n(response->mutable_neighbor_weights()->mutable_data() + offset, count,
            ...
            {
                std::lock_guard<std::mutex> lock{mutex};
                response->add_node_ids(node_index);
            }

            auto &last_shard_weight = response->mutable_shard_weights()->at(node_index);
            for (size_t partition = 0; partition < partition_count; ++partition)
            {
                                                            ...
                                                            snark::UniformSampleNeighborsReply *response)
{
    ...
    response->mutable_shard_counts()->Resize(node_size, {});
    response->mutable_neighbor_ids()->Resize(node_size * count, 0);
    response->mutable_neighbor_types()->Resize(node_size * count, 0);
    ...
    auto neighbor_func = [this, &request, &count, &response, &mutex, &seed, &input_edge_types, &without_replacement](
        ...
        for (size_t node_index = start_node_id; node_index < end_node_id; ++node_index)
        {
            ...
            auto &last_shard_weight = response->mutable_shard_counts()->at(node_index);
            for (size_t partition = 0; partition < partition_count; ++partition)
            {
